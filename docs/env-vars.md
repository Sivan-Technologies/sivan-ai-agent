...
- `PAYOUT_VERIFICATION_TEST_MODE` - Controlled E2E test override. Set `true` only while using a Paystack `sk_test_...` key. It allows exact-account payout verification and creates a clearly marked sandbox payment reference when Paystack test transaction initialization is unavailable. It never activates with live Paystack credentials.
