...
| Phase 1B | Provider-neutral Naira payment transports | 🟡 Implemented / final provider proofs pending | `src/services/nairaPaymentProvider.ts` defines the provider-neutral Naira collection contract. Paystack, Monnify, and Flutterwave now run behind it without changing existing escrow rules. Monnify supports token caching, transfer-only initialization, payment verification, signed webhook handling, duplicate-safe event persistence, and settlement event visibility in Revenue/Reconciliation analytics; paid sandbox transfer verification passed on 2026-06-11. Flutterwave supports emergency dynamic virtual-account collection, signed webhook handling, and server-side charge verification, all bank-transfer only. Flutterwave fails closed instead of falling back to hosted checkout so card payment UI cannot enter the Sivan flow. Admin Platform Controls can set active/backup/emergency providers, platform mode, maintenance message, and bank-transfer-only policy with audit history. Local provider tests passed on 2026-06-19. Remaining work is Monnify signed webhook delivery proof, Flutterwave low-value backup proof, Flutterwave settlement reconciliation, and future PalmPay adapter |
| Phase 2 | WhatsApp bot bridge | ✅ Completed for MVP | Twilio auth and Meta Cloud API paths exist; backend-authorized Deal Cards, participant-filtered My Deals, durable active-deal context, no-ID lifecycle commands, multiple-deal selection, concise customer copy, confirmations for cancel/release/dispute, malformed Twilio `ButtonPayload` recovery, and visible seller-invite button recovery are implemented |
...
| Phase 10B | Seller delivery proof workflow | 🟡 Lean MVP implemented | Seller `deliver SIV-...` is available only after funding, starts a private proof session in WhatsApp, accepts text/media metadata, stores `seller_delivery_proof_recorded` in the escrow event timeline, notifies the buyer with the current Deal Card, and blocks external links by default through `DELIVERY_PROOF_ALLOW_EXTERNAL_LINKS=false`. Remaining work is richer admin proof viewing, durable media storage/virus scanning, and buyer-specific review/problem quick replies |
...
| Phase 14 | Cross-repo production operations | 🟡 In progress | Escrow backend, WhatsApp bot, Telegram admin auth, admin frontend, and public homepage build and test cleanly. GitHub CLI auth and exact Actions smoke secrets were verified, manual workflow run `26961031020` passed, scheduled runs are green, and live smoke/incident/DR checks passed. A temporary 5-minute GitHub Actions uptime ping now keeps the Render Hobby WhatsApp bot and backend health endpoints warm during pilot testing; remove it after upgrading those services to always-on paid infrastructure. The WhatsApp bot's legacy/text fallback was verified live on 2026-06-04, and backend participant reads now degrade to stored agreement records when payment-lifecycle refresh fails so `MY DEALS` and `STATUS SIV-...` do not collapse into known-reference-only fallbacks. Twilio sandbox/trial account daily message limit `63038` remains an operational limitation, not a code failure. The homepage is ✅ complete for controlled MVP launch: apex redirects to `www`, Vercel serves the canonical domain, SEO/social assets are present, and production-preview Lighthouse scored 98/100/100/100 on 2026-06-05. Remaining work is keeping the deployed backend participant `/api/users/escrows` contract verified after deploys, rotating the formerly exposed admin smoke key, Meta live webhook verification, legal counsel review of public Terms/Privacy before scaled real-money use, and keeping operational proof current after deploys |
...
- 🟡 WhatsApp seller delivery proof workflow: `deliver SIV-...`, awaiting-proof session, private media/document metadata intake, buyer notification, escrow timeline event, and no-link MVP policy are implemented; richer proof review UI and durable media storage remain next
...
- ✅ temporary 5-minute GitHub Actions uptime ping for Render Hobby pilot testing, with `SIVAN_UPTIME_PING_ENABLED=false` as the clean disable path before deleting it on paid infrastructure
...
| Alert routing | ✅ Completed for Telegram/webhook/Sentry MVP | Payment and operational warnings are captured in memory, sent to Sentry when configured, and can be forwarded to Telegram with `OPERATIONS_ALERT_PROVIDER=telegram` or to `OPERATIONS_ALERT_WEBHOOK_URL`; Telegram routing separates customer-impact ops alerts from provider/security/debug alerts |
...
| Abuse prevention foundation | ✅ Completed for MVP | Escrow creation risk scoring, request/device fingerprint metadata, abuse signal persistence, high-risk blocking, reputation watchlist, velocity dashboard, operator analytics, and database-backed buyer-tier/buyer-exposure/platform-exposure creation limits exist |
...
| Compliance MVP | ✅ Completed for risk-gated MVP | Seller payout setup stores resolved account name, name-match score/level, verification provider/time, shared-account count/flag, and release readiness blocks weak/failed/shared/high-value payout paths. Admin Platform Controls can adjust audited Naira limits, while Admin Limit Reviews can approve or reject a single blocked creation request without changing global policy |
...
| MVP dispute operations policy | ✅ Documented | `docs/dispute-mvp.md` defines manual-only decisions for the first 200 users, evidence handling, deadlines, supported outcomes, high-value escalation, AI restrictions, and the remaining maker-checker/refund-proof controls |
