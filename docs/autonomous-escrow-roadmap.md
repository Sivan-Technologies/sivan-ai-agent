...
| Backup and disaster recovery | ✅ Foundation completed / 🟡 live restore drill required | `/admin/dr/status`, Admin Ops DR visibility, `.env`-aware `npm run dr:check`, and `docs/disaster-recovery.md` exist; live DR check now confirms backup/rollback/outage readiness, and the next step is creating a Neon restore target, performing a staging restore, and recording proof in release notes |
...
4. 🟡 Run the first live database restore drill on a staging database, update `BACKUP_LAST_RESTORE_TEST_AT`, and store proof in release notes. Blocked until a Neon restore/staging database URL or Neon API credentials are available.
